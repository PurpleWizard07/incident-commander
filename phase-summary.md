...
> **Phase 3 — code complete and deployed; blocked on the one human-only step.** All 12 investigation
> tools are registered and live on production. Everything programmatically verifiable has been
> verified (1.5K budget against synthetic worst-case data, empty-result pointers, live API responses
> matching what each tool formatter expects). What's left needs you: open
> **https://incident-commander-461.netlify.app** in the **ChatGPT in-app browser** and ask
> *"Investigate the checkout incident and tell me what you recommend."* Correct answer: **checkout-v3
> deployment** — not payments, which looks worse (83% vs 64% error rate) but is downstream. Tell me
> what the agent says and I'll close out Phase 3 and start Phase 4 (console shell).
>
> Once that's confirmed, Chrome + flag (P1) is worth a quick check too, same question, same page —
> not blocking, same pattern as Phase 0.
...
| 2026-08-31 | 3 | **Every tool returns a compact plain-text string (via `toolResult()`, a thin `{content:[{type:'text',text}]}` wrapper), never nested JSON.** `apps/web/src/webmcp/tools/shared.ts`. | More token-efficient for the agent, and matches how the concept doc's own worked examples ("Added todo item: X successfully") communicate — natural-language summaries, not data structures, are what these tools are for. Also makes the 1.5K budget trivially measurable as `string.length` |
| 2026-08-31 | 3 | **Shaping logic (log/trace/metric reformatting) is a set of pure, separately-exported functions (`formatLogsResult`, `formatTracesResult`, `formatCompareResult`), deliberately split from the `fetch` in each tool's `execute()`.** `apps/web/src/webmcp/tools/observability.ts`. | Lets the 1.5K budget be tested against **synthetic worst-case inputs** (200 patterns, 20×10 spans, all 7×13 service/metric pairs) rather than only today's actual scenario data — proves the cap holds structurally for any future scenario's data volume, not just by observed accident. Test: `apps/web/test/shape-budget.test.ts` |
| 2026-08-31 | 3 | **`compare_metrics` bug found and fixed in the Phase 2 API handler:** was defaulting to comparing all 7 services when `services` was omitted; plan §6.3's own tool description says "Omit for all affected services" — meaning the *incident's* affected services (3, for INC-4821), not literally every service in the system. `routes/observability.ts` now reads `session.incidents[0].affectedServices` as the default. | Caught by re-reading the plan's exact tool-description wording while writing the frontend tool wrapper, not by testing — a reminder that the API's behavior needs to match what the TOOL DESCRIPTION promises the agent, which is a different check than "does the endpoint return data" |
| 2026-08-31 | 3 | **`apps/web/src/vite-env.d.ts` now carries an explicit `/// <reference path=".../webmcp-dom.d.ts">`, rather than relying on some other file's import of `@incident-commander/shared` to transitively pull the ambient `document.modelContext` types in.** | Phase 0's version worked by accident — `registerTools.ts` happened to import `SERVICE_IDS` from shared, which pulled the ambient types in as a side effect. The Phase 3 rewrite imports from local `./tools/*.js` files instead and stopped importing shared at all, silently breaking the ambient type chain (typecheck failures only, not a runtime bug, but instructive: an ambient-type dependency on "something transitively imports this" is fragile and worth making explicit) |
| 2026-08-31 | 3 | **`nowMinute` cannot currently be varied via the API** — a session bootstraps at the scenario's fixed `defaultNowMinute` and there is no lever to ask "what did the world look like at minute 50" through the deployed API (only `packages/sim`'s own `materializeWorld` function takes `nowMinute` as a parameter directly). Phase 3's "under 1.5K at every incident minute" exit criterion is therefore verified two different ways: structurally via synthetic-worst-case unit tests (any volume), and empirically against the one reachable live value (minute 93, this scenario's highest-volume point). | Full point-in-time API querying is a Phase 6/7 concern (the virtual clock advancing during a live session) — not a gap to fix now, just worth being precise about what "verified" actually means at this point |
